...
def jetToolbox( proc, jetType, jetSequence, outputFile,
  ...
  if not updateCollection:
    ...
    if miniAOD:
      ...
      if runOnMC:
        ...
        mod['selectedGenJetsNoNu'] = 'selected'+mod['GenJetsNoNu']
        _addProcessAndTask(proc, mod['selectedGenJetsNoNu'], \
            cms.EDFilter("GenJetSelector",
              src = cms.InputTag(mod['GenJetsNoNu']),
              cut = cms.string(genJetCut),
              filter = cms.bool(False)
            )
        )
        jetSeq += getattr(proc, mod['selectedGenJetsNoNu'])
    ...
    if ( PUMethod in [ 'CHS', 'CS', 'Puppi' ] ) and miniAOD: 
      _addProcessAndTask( proc, mod["PFJetsConstituents"], cms.EDProducer("MiniAODJetConstituentSelector", src = cms.InputTag( mod["PFJets"] ), cut = cms.string( "pt>30" ) ))
    else: 
      _addProcessAndTask( proc, mod["PFJetsConstituents"], cms.EDProducer("PFJetConstituentSelector", src = cms.InputTag( mod["PFJets"] ), cut = cms.string( "pt>30" ) ))
    ...
    addJetCollection(
        ...
        genJetCollection = cms.InputTag(mod["selectedGenJetsNoNu"]),
  ...
  if dataTier.startswith('nanoAOD'):
            ...
            if runOnMC:
                ...
                mod['jetToolboxGenJetTable'] = mod['selectedGenJetsNoNu']+'JTBTable'
                _addProcessAndTask( proc, mod['jetToolboxGenJetTable'], genJetTable.clone(
                    src = cms.InputTag(mod["selectedGenJetsNoNu"]),
                    name = cms.string(mod["selectedGenJetsNoNu"]),
